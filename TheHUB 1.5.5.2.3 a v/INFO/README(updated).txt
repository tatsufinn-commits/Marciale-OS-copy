...
Idle Hero is the full Momentum Companion webgame page. ChessLab now includes the Build 30 local board foundation plus offline local opponent, real browser-side rules validation, chess engine worker foundation, Stockfish browser integration, Elo-shaped AI difficulty controls, character-based local opponents, a special Marciale bot with scalable engine-strength modes up to 3800, local chess sound effects, a personality-biased move policy layer, upgraded bot-select UX with records and premium Marciale presentation, the Marciale Chess Coach layer with engine-backed candidate summaries, a calibrated Elo behavior layer to reduce random collapse at lower strengths, Hub Activity / Companion bridge, and Marciale tactical analysis prompt, while RuView remains a prepared Category C page for later builds.
